...
<cfif structKeyExists(form, "statusFilter")>
    <cfset session.documents_statusFilter = form.statusFilter>
    <cflocation url="index.cfm" addtoken="false">
</cfif>

<!--- Initialize session filter if not set --->
<cfif NOT structKeyExists(session, "documents_statusFilter")>
    <cfset session.documents_statusFilter = "all">
</cfif>
...
<cfquery name="qDocuments" datasource="#application.datasource#">
    ...
    FROM dbo.documents
    WHERE 1=1
    <cfif session.documents_statusFilter NEQ "all">
        AND is_active = <cfqueryparam value="#session.documents_statusFilter EQ 'active' ? 1 : 0#" cfsqltype="cf_sql_bit">
    </cfif>
                    ...
                    <div class="search-container">
                        ...
                        <form method="post" action="index.cfm" style="display: inline;">
                            <select name="statusFilter" class="status-filter" onchange="this.form.submit()">
                                <option value="all" <cfif session.documents_statusFilter EQ "all">selected</cfif>>All Statuses</option>
                                <option value="active" <cfif session.documents_statusFilter EQ "active">selected</cfif>>Active</option>
                                <option value="inactive" <cfif session.documents_statusFilter EQ "inactive">selected</cfif>>Inactive</option>
                            </select>
                        </form>
                    ...
                    <table class="data-table" id="documents-table">
                        ...
                        <tbody id="tableBody">
                            <cfoutput query="qDocuments">
                                <tr data-search="#lCase(htmlEditFormat(title))# #lCase(htmlEditFormat(description))# #lCase(htmlEditFormat(original_filename))#" data-status="#is_active ? 'active' : 'inactive'#">
