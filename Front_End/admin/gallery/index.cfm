...
<cfif structKeyExists(form, "statusFilter")>
    <cfset session["gallery_#url.location#_statusFilter"] = form.statusFilter>
    <cflocation url="index.cfm?location=#url.location#" addtoken="false">
</cfif>

<!--- Initialize session filter for current tab --->
<cfif NOT structKeyExists(session, "gallery_#url.location#_statusFilter")>
    <cfset session["gallery_#url.location#_statusFilter"] = "all">
</cfif>
...
<cfquery name="qImages" datasource="#application.datasource#">
    ...
    <cfif session["gallery_#url.location#_statusFilter"] NEQ "all">
        AND is_active = <cfqueryparam value="#session['gallery_#url.location#_statusFilter'] EQ 'active' ? 1 : 0#" cfsqltype="cf_sql_bit">
    </cfif>
                    ...
                    <div class="search-container">
                        ...
                        <form method="post" action="index.cfm?location=#url.location#" style="display: inline;">
                            <select name="statusFilter" class="status-filter" onchange="this.form.submit()">
                                <option value="all" <cfif session["gallery_#url.location#_statusFilter"] EQ "all">selected</cfif>>All Statuses</option>
                                <option value="active" <cfif session["gallery_#url.location#_statusFilter"] EQ "active">selected</cfif>>Active</option>
                                <option value="inactive" <cfif session["gallery_#url.location#_statusFilter"] EQ "inactive">selected</cfif>>Inactive</option>
                            </select>
                        </form>
                    ...
                    <table class="data-table" id="gallery-table">
                        ...
                        <tbody id="tableBody">
                            <cfoutput query="qImages">
                                <tr data-search="#lCase(htmlEditFormat(title))# #lCase(htmlEditFormat(original_filename))#" data-status="#is_active ? 'active' : 'inactive'#">
