...
function Activate() {
  ...
  const { activateAccount, checkAuthStatus } = useAuth();
  ...
  useEffect(() => {
    ...
    setToken(tokenParam);
    if (!tokenParam) {
      setErrorMessage('Invalid activation link. Please contact the administrator.');
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await activateAccount(token, newPassword);

      if (response.success) {
        // Backend now auto-logs user in after successful activation
        if (response.user) {
          // Trigger a re-check to get the session
          checkAuthStatus();
        }

        setSuccessMessage('Account activated successfully! You are now logged in. Redirecting to dashboard...');
        setTimeout(() => {
          navigate('/admin/dashboard');
        }, 2000);
      } else {
        setErrorMessage(response.message || 'Failed to activate account.');
        setLoading(false);
      }
    } catch (error) {
      setErrorMessage(error.message || 'Failed to activate account. Please try again or contact the administrator.');
      setLoading(false);
    }
  };
    ...
    <div className="admin-login-page">
      <div className="login-container">
        ...
        <div className="login-footer">
          <Link to="/admin/login?from=activate" className="back-link">← Back to Login</Link>
        </div>
        </div>
      </div>
    </div>
